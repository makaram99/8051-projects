...
This is a simple traffic lights system, using the **8051 (8-bit) microcontroller**.
The traffic light system is a multi-state (Timed) system which means it has different states for example RED state, RED-YELLOW state, GREEN state, and YELLO state.
Transitions between states are defined by time passing, and no input is required.

<p align="center">
  <img height="300" src="docs/traffic_lights.png" alt="logo">
</p>
...
1. The RED light is on for 4 seconds.
2. The RED-YELLOW light is on for 2 seconds.
3. The GREEN light is on for 4 seconds.
4. The YELLOW light is on for 2 seconds.
5. The system goes back to the RED state (step 1).
...
- Click twice on **[traffic_lights.uvproj](code/traffic_lights.uvproj)** file.
