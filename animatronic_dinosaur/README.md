<h1 align="center">Animatronic Dinosaur</h1>
...
This is a simple Animation for a Dinosaur in the Nature History Museum in Lindon, using the **8051 (8-bit) microcontroller**.
The Animatronic Dinosaur is a multi-state (Time) system which means it has different states for example sleeping, waking, growling, and attacking.
Each state has a different behaviour. Transitions between states are defined only by time passing, and no inputs are used.

<p align="center">
  <img height="300" src="docs/dinosaur_states.png" alt="logo">
</p>
...
Read this [guide](https://github.com/makaram99/8051-projects/tools.md) to know what tools are required to develop this project.
...
### System Behaviour
The animation works as the following:
1. Sleeping: The dinosaur will be largely motionless, but will be obviously ‘breathing’. Irregular snoring noises, or slight movements during this time will add interest for the audience. Sleeping time is defined in \ref SLEEPING_DURATION macro.
2. Waking: The dinosaur will begin to wake up. Eyelids will begin to flicker. Breathing will become more rapid. Waking time is defined in \ref WAKING_DURATION macro.
3. Growling: Eyes will suddenly open, and the dinosaur will emit a very loud growl. Some further movement and growling will follow. Growling time is defined in \ref GROWLING_DURATION macro.
4. Attacking: Rapid ‘random’ movements towards the audience. Lots of noise (you should be able to hear this from the next floor in the museum).Attack time is defined in \ref ATTACK_DURATION macro.

To make simulation on Proteus more easy, we will consider the working of each state as in the following table.

<p align="center">
  <img height="300" src="docs/simulation_table.BMP" alt="logo">
</p>
...
* Click twice on **[animatronics_dinosaur.uvproj](code/animatronics_dinosaur.uvproj)** file.
...
2. From the home directory of the project ([animatronics_dinosaur](./)), open the command prompt in your OS (**cmd** for Windows, **bash** for linux, ...)
